...
class MCPService(
  ...
  fun getAvailableTools(): List<MCPTool> {
    return toolRegistry.allTools.map { tool ->
      MCPTool(
  ...
  suspend fun executeTool(request: MCPToolRequest): MCPToolResult {
    return when (request.name) {
      ...
      else -> createErrorResult("Неизвестный инструмент: ${request.name}")
    }
  }
  ...
  suspend fun queryWithKnowledgeBase(
    ...
    return executeQueryWithKnowledgeBase(question, userId)
  }
  ...
  suspend fun queryWithoutKnowledgeBase(
    ...
    return MCPToolResult(content = listOf(MCPContent(type = "text", text = answer)))
  }
